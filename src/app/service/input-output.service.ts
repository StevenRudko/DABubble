...
export class InputOutput {
  ...
  async getUserMessages() {
    try {
      ...
      const messages: UserMessage[] = querySnapshot.docs.map(
        (doc) =>
          new UserMessage({
      ...
      onSnapshot(userCollection, (snapshot) => {
        const updatedMessages: UserMessage[] = snapshot.docs.map(
          (doc) =>
            new UserMessage({
              id: doc.id, // Firebase ID als `id` setzen
              ...doc.data(), // Restliche Daten des Dokuments
        ...
        console.log(
          'Echtzeit-Updates der Benutzer-Nachrichten:',
          updatedMessages
        );
      });
    } catch (error) {
